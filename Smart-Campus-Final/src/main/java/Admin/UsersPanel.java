...
public class UsersPanel extends javax.swing.JPanel {
  ...
  private void addRow(User1 user) {
    tbmUsers.addRow(new Object[]{
      user.getUser_id(), user.getUsername()
    });
  }

  private void addRows(java.util.ArrayList<User1> users) {
    tbmUsers.setRowCount(0);
    users.forEach(user -> addRow(user));
    resizeColumns();
  }

  private void updateRow(User1 users) {
    javax.swing.table.TableModel model = (javax.swing.table.TableModel) tblUsers.getModel();
    model.setValueAt(users.getUser_id(), currentRowIndex, 0);
    model.setValueAt(users.getUsername(), currentRowIndex, 1);
  }
  ...
  private void populateFields(User1 users) {
  ...
  private void getAllUsers() {
    java.util.ArrayList<User1> users = userService.getAll();
    if (users.size() > 0) {
      addRows(users);
    }
  }
  ...
  private void getOneUser(int id) {
    User1 user = userService.getOne(id);
    if (user != null) {
      populateFields(user);
    }
  }

  private void createOneUser(User1 users) {
  ...
  private void updateOneItem(User1 user) {
